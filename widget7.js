(function () {
  console.log("🤖 Chatbot Widget v2 Loaded (Final Fix)");
  ...
  async function init() {
    ...
    async function trackQuestionUsage(questionText) {
      try {
        ...
        console.log("📌 FAQ MATCH RESULT:", data, error);

        if (error || !data?.id) return;

        const { error: insertError } = await sb
          .from("faq_question_usage")
          .insert({
            customer_id,
            question_id: data.id,
            user_id
    ...
    async function fetchSuggestions(keyword) {
      ...
      if (data?.length) {
        ...
        suggestionsBox.innerHTML =
          "<div style='padding:6px;color:#888'>No matches</div>";
      }
    }
