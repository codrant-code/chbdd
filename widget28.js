(function () {
  ...
  async function trackUsage(question_id) {
    ...
    await api("track_faq_usage", "POST", {
      customer_id,
      user_id,
      question_id: String(question_id) // 🔥 ensure string safety
    });
  }
  ...
  function renderSuggestions(data) {
    ...
    data.forEach((item) => {
      ...
      div.onclick = async () => {
        ...
        // 🔥 FIX: ensure fallback if id missing
        await trackUsage(item.id || item.question);
  ...
  async function sendMessage() {
    ...
    // 🔥 FIX: stable tracking (avoid undefined IDs)
    await trackUsage("typed_" + msg.toLowerCase());
