(function () {
  console.log("🤖 Chatbot Widget v2 Loaded (Fixed)");
  ...
  async function init() {
    ...
    // =========================
    // TRACK USAGE (FIXED - NO UPSERT ERROR)
    // =========================
    async function trackQuestionUsage(questionText) {
      try {
        ...
        // ✔ SAFE INSERT (works with unique index)
        await sb.from("faq_question_usage").insert({
          customer_id,
          question_id: data.id,
          user_id
        });
        ...
        // ignore duplicates (expected behavior)
        console.log("Tracking skipped (likely duplicate user-question)");
      }
    }
    ...
    // =========================
    // SEND MESSAGE (WITH TRACKING FIXED)
    // =========================
